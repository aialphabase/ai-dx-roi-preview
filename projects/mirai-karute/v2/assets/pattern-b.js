(function () {
  ...
  const readState = () => {
    ...
    return {
      ...
      tenureYears: Math.max(0, n("b-tenure")),
      otherLoans: Math.max(0, n("b-loans")),
      propertyInterest: byId("b-property") ? byId("b-property").value : "none",
    };
  };
  ...
  const buildOps = (state, diagnosis, scores) => {
    ...
    const ops = {
      ...
      answers: {
        ...
        home_planned: state.homeEnabled,
        home_age: state.homeEnabled ? state.homeAge : null,
        home_cost_man: state.homeEnabled ? state.homeCost : null,
        children: state.educationEnabled ? state.children : 0
      },
      screening: {
        income_band: state.annualIncome > 0 ? `世帯手取り年収 約${state.annualIncome}万円` : null,
    ...
    window.__ops = ops;
    try { localStorage.setItem("mirai_karte_ops_b", JSON.stringify(ops)); } catch (e) {}
